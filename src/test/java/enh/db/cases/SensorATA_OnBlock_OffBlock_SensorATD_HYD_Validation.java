...
public class SensorATA_OnBlock_OffBlock_SensorATD_HYD_Validation{
	...
	public static void SensorATA_LessThanOnBlock_LessThanOffBlock_LessThanSensorATD_HYD_Report(int operationunit) throws Exception{
		...
		ResultSet result1 = DBWrapper.Connect("SELECT count(*) FROM `DailyFlightScheduleDeparture_GMR` where \r\n" +
				"date(IFNULL(std, mediator_std))='"+SQL_Queries.yesterDate()+"' and operationunit= "+operationunit+"");
		while (result1.next())
		{				
		...
		email_report_SensorATA_OnBlock_OffBlock_SensorATD_For_Hyd1.append("<style>table#t01, th, td {border: 1px solid black;border-collapse: collapse;}table#t01 th{background-color:#80e5ff; color: #000000;} table#t01 tr:nth-child(even) {background-color: #f2f2f2;} table#t01 tr:nth-child(odd) { background-color: #DFEDEC;}table#t01 th, td {padding: 5px;}table#t01 th,td {text-align: center;} table#t01 caption {color: #336600;font-weight: bold;}</style>");

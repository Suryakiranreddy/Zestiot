...
import java.util.Date;
	...
	public static String strQuery_017="SELECT count(*) FROM `DailyFlightScheduleArrival_GMR`where (date(sta)= '"+SQL_Queries.yesterDate()+"' or date(mediator_sta)='"+SQL_Queries.yesterDate()+"') and operationunit= 4";
	
	//Total flights count for GMR-Hyd for which sensor_ata has been captured
	public static String strQuery_018="SELECT count(*) FROM `DailyFlightSchedule_Merged` where flightArrivalId in (SELECT logid FROM `DailyFlightScheduleArrival_GMR`where (date(sta)= '"+SQL_Queries.yesterDate()+"' or date(mediator_sta)='"+SQL_Queries.yesterDate()+"') and operationunit=4) and sensor_ata is not null";
	
	//Total flights count for GMR-Hyd for which sensor_ata has not been captured
	public static String strQuery_019="SELECT count(*) FROM `DailyFlightSchedule_Merged` where flightArrivalId in (SELECT logid FROM `DailyFlightScheduleArrival_GMR`where (date(sta)= '"+SQL_Queries.yesterDate()+"' or date(mediator_sta)='"+SQL_Queries.yesterDate()+"') and operationunit=4) and sensor_ata is null";
	
	//Total flights for GMR-Hyd for which sensor_ata has not been captured
	public static String strQuery_020="SELECT logid, flightnumber_arrival, flightnumber_departure, sensor_ata FROM `DailyFlightSchedule_Merged` where flightArrivalId in (SELECT logid FROM `DailyFlightScheduleArrival_GMR`where (date(sta)= '"+SQL_Queries.yesterDate()+"' or date(mediator_sta)='"+SQL_Queries.yesterDate()+"') and operationunit=4) and sensor_ata is null";

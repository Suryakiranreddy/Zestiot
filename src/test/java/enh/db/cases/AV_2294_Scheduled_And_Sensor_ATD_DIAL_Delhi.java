...
public class AV_2294_Scheduled_And_Sensor_ATD_DIAL_Delhi {
	...
	public static void scheduledAndSensorATDForDelhi_Report(int operationunit,String environment) throws Exception{
		...
		ResultSet result1 = DBWrapper.Connect("SELECT count(*) FROM `DailyFlightSchedule_Merged` where gmrpk_departure in (SELECT gmrpk FROM `DailyFlightScheduleDeparture_GMR` where\r\n" + 
						"date(IFNULL(std, etd))= '"+SQL_Queries.yesterDate()+"' and operationunit= "+operationunit+") and (atd is not null or sensor_atd is not null)",environment);
				while (result1.next())
				{				
		...
		ResultSet result5 = DBWrapper.Connect("SELECT logid, flightnumber_departure, sensor_ATD, Off_block_time, (case when (Off_Block_Time < Sensor_ATD) then 1 else 0 end) as Status, \r\n"
				...
				email_report_Scheduled_And_Sensor_ATD_For_Delhi1.append("<h5 align=\"center\" style=\"color:#008ae6;\" >Environment: "+environment+" </h5>");
				...
				emailbody.append("<h4 align=\"center\" style=\"color:#008ae6;\">Executed For :Scheduled and Sensor-ATD for DIAL-Delhi</h4>");

...
public class AV_2294_Scheduled_And_Sensor_ATD_Delhi_CELEBI_User {
	...
	public static void scheduledAndSensorATDForCELEBI_Delhi_Report(int operationunit) throws Exception{
		...
		ResultSet result3 = DBWrapper.Connect("SELECT logid, flightnumber_departure, std, etd, atd FROM `DailyFlightSchedule_Merged` where date(IFNULL(std, etd))= '"+SQL_Queries.yesterDate()+"' \r\n"
						+ "and flightnumber_departure regexp ('TK|EY|CX|QR|ET|AK|KC|JL|KE|PS|AC|AF|AZ|BA|SU|IA|SV|UL|T5|FZ|KQ|I5') and operationunit= "+operationunit+" and sensor_atd is null");
				while (result3.next())
				{
		...
		ResultSet result4 = DBWrapper.Connect("SELECT count(*) FROM `EquipActivityLogs` where flight_pk in (SELECT logid FROM `DailyFlightSchedule_Merged`\r\n" + 
				...
				"and flightnumber_departure regexp ('TK|EY|CX|QR|ET|AK|KC|JL|KE|PS|AC|AF|AZ|BA|SU|IA|SV|UL|T5|FZ|KQ|I5')) and operationname = 'ofb' and type = 'aircraft'");
						while (result4.next())
						{				
		...
		ResultSet result5 = DBWrapper.Connect("SELECT logid, flightnumber_departure, sensor_ATD, Off_block_time, (case when (Off_Block_Time < Sensor_ATD) then 1 else 0 end) as Status, \r\n"
				...
				+ "and flightnumber_departure regexp ('TK|EY|CX|QR|ET|AK|KC|JL|KE|PS|AC|AF|AZ|BA|SU|IA|SV|UL|T5|FZ|KQ|I5') and operationunit = "+operationunit+" and (sensor_atd is not null and Off_block_time is not null) order by flightnumber_departure");
						while (result5.next())
						{				
				...
				email_report_Scheduled_And_Sensor_ATD_For_CELEBI_Delhi1.append("<h4 align=\"center\" style=\"color:#008ae6;\">Executed For :Scheduled and Sensor-ATD</h4><h5 align=\"center\" style=\"color:#008ae6;\" >Execution Time: "+SQL_Queries.todayDayDateTime()+" </h5>");
